...
public class HotelService {
    ...
    public HotelService(RoomRepository roomRepository, PaymentService paymentService, InvoiceRepository invoiceRepository) {
    ...
    public Invoice produceInvoice(String customerName, LocalDate endDate, List<String> roomNumbers) {
        ...
        Map<String, List<LineItem>> lineItemsPerRoom = bookingsForRooms.entrySet()
                .stream()
                .collect(Collectors.toMap(
                        Map.Entry::getKey,
                        entry -> convert(entry.getValue())
                ));
        Invoice invoice = paymentService.produceInvoice(customerName, totalAmount, lineItemsPerRoom);

        roomRepository.markBookingsAsInvoiced(bookingsForRooms, invoice.getId());
    ...
    private List<LineItem> convert(List<BookingInterval> bookingIntervals) {
        return bookingIntervals.stream()
                .map(b -> new LineItem(b.getCustomerName() + ": " +b.getStartDate() + " - " +b.getEndDate()))
                .toList();
    }
}
